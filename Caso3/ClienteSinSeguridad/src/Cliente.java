...
public class Cliente {
	...
	private void etapa2() throws Exception {
		...
		if ((fromServer = lector.readLine()) != null) {
			...
			if (fromServer.equals("OK") && (fromServer = lector.readLine()) != null) {
				// System.out.println("Certificado del Servidor:+(certificadoServidor =
				// Seguridad.generarCertificado(fromServer)).toString() + "\n");
				...
				escritor.println((llaveSimetrica = Seguridad.generarLlaveSimetrica(algoritmos)).getEncoded());

				String reto = Seguridad.completar("reto");
				escritor.println(DatatypeConverter.printBase64Binary(reto.getBytes()));
				if ((fromServer = lector.readLine()) != null) {
					String line;
					// System.out.println("Verificando...");
					if (reto.equals(new String(DatatypeConverter.parseBase64Binary(fromServer))))
	...
	private void etapa3() throws IOException, InvalidKeyException, NoSuchAlgorithmException, NoSuchPaddingException,
		...
		escritor.println(DatatypeConverter.printBase64Binary((Seguridad.completar("123456789")).getBytes()));
		escritor.println(DatatypeConverter.printBase64Binary((Seguridad.completar("123456").getBytes())));
	...
	private void etapa4() throws Exception {
		...
		if ((fromServer = lector.readLine()) != null) {
			...
			if ((fromServer = lector.readLine()) != null) {
				...
				if ((valor.hashCode()+"").equals(fromServer))
					escritor.println(line = "OK");
				else
					escritor.println(line = "ERROR");
				// System.out.println(line);
