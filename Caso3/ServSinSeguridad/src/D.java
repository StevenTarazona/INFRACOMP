...
public class D extends Thread {
	...
	private boolean validoAlgHMAC(String nombre) {
		return ((nombre.equals(S.HMACMD5) || nombre.equals(S.HMACSHA1) || nombre.equals(S.HMACSHA256)
				|| nombre.equals(S.HMACSHA384) || nombre.equals(S.HMACSHA512)));
	}

	/*
	 * Generacion del archivo log. Nota: - Debe conservar el metodo como está. - Es
	 * el único metodo permitido para escribir en el log.
	 */
	...
	public void run() {
		...
		try {
			...
			if (!algoritmos[1].equals(S.DES) && !algoritmos[1].equals(S.AES) && !algoritmos[1].equals(S.BLOWFISH)
					&& !algoritmos[1].equals(S.RC4)) {
				ac.println(ERROR);
				sc.close();
				P.perdidas();
			...
			/***** Fase 5: *****/
			cadenas[4] = "";
			linea = new String(DatatypeConverter.parseBase64Binary(dc.readLine()));
			System.out.println(dlg + "Recibio reto del cliente:-" + linea + "-");
			ac.println(DatatypeConverter.printBase64Binary(linea.getBytes()));
			...
			/***** Fase 6: *****/
			linea = dc.readLine();
			String cc = new String(DatatypeConverter.parseBase64Binary(linea));
			System.out.println(dlg + "recibio cc y descifro:-" + cc + "-continuado.");

			linea = dc.readLine();
			String clave = new String(DatatypeConverter.parseBase64Binary(linea));
			...
			ac.println(toHexString(toByteArray(strvalor.hashCode()+"")));

			//escribe resultados en el csv
